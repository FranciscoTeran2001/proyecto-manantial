...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (dataUser !== "") {
        ...
        if (dataUser.ownerType === "admin") {
          navigate({ pathname: "/OwnerController" }, { state: { dataUser } });
        } else if (dataUser.ownerType === "owner") {
          navigate({ pathname: "/home", hash: dataUser.id }, { state: { dataUser } });
        }
      }
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Box sx={{ display: "flex", flexWrap: "wrap" }} className="LogIn-header">
